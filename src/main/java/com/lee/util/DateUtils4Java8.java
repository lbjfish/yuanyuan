...
public class DateUtils4Java8 {
    ...
    public static void main(String[] args) {
        long abc = getSubDays("2018-08-10", "2018-08-13");
        LocalDate startDate = LocalDate.parse("2018-08-10");
        LocalDate endDate = LocalDate.parse("2020-01-17");
        System.out.println(abc);

        LocalDate now = LocalDate.now();
        Long between = ChronoUnit.DAYS.between(now , endDate);
        System.out.println(between);

        boolean ba = now.isBefore(endDate);
        System.out.println(ba);
        boolean ba2 = now.isAfter(endDate);
        System.out.println(ba2);

        long asd = endDate.until(now, ChronoUnit.DAYS);
